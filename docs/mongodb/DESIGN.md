...
| 9.23 | Đăng nhập sai: **web chỉ báo "Login failed"**, không hiện lý do thật (400 `Email dang nhap khong dung dinh dang`, 400 `Sai tên đăng nhập hoặc mật khẩu`, 403 `Tài khoản bị từ chối đăng nhập`, 500…) | `ApiService.Auth.loginJwt` **tự bọc lỗi**: `if (!response.ok) throw new Error('Login failed')` ⇒ **nuốt** message thật của server; ngoài ra `handleResponse()` chỉ đọc body dạng text nên JSON `{message}` cũng bị biến thành `'Request failed'` | `loginJwt` dùng `handleResponse(response)` (giữ `error.status`/`error.data`) + `trim()` email trước khi gửi; thêm `extractErrorMessage()` đọc cả body text **và** JSON `{message\|error}` |
| 9.24 | `admin@gmail.com / Admin123@` (tài khoản ghi trong README) trả **400** `Sai tên đăng nhập hoặc mật khẩu` dù "mật khẩu đúng" | Instance MongoDB 27018 vẫn giữ **dữ liệu seed cũ**: 241 user có `passwordHash = "$2a$10$seedHash<id>"` (**hash giả**, không khớp mật khẩu nào) và username **không phải email** (`admin`, `seller2`, `buyer300`); `db/mongo/11_import_sellers.js` chưa từng chạy ⇒ `admin@gmail.com` + 100 nhà bán thật **không tồn tại** | Xoá 241 user hash giả (`db.users.deleteMany({passwordHash:/seedHash/})`) rồi chạy `tools\mongo-import-sellers.bat` ⇒ `admin@gmail.com/Admin123@` (\_id=1509) + 100 nhà bán (username = email, hash `$2a$12$` thật); kiểm chứng `POST /api/auth/login-jwt` → **200** |
| 9.25 | Tài khoản có `username ≠ email` (username `admin`, email `admin@bookom.vn`) **không thể** đăng nhập bằng email; tài khoản tạo bằng Google (`passwordHash = ""`) làm `login-jwt` trả **500 `StringIndexOutOfBoundsException`** | `AuthService.authenticateUser()` chỉ tra `findByUsername`; gọi `BCrypt.checkpw` với hash **rỗng** hoặc **sai định dạng** (`$2a$10$seedHash1` — 16 ký tự, jbcrypt đọc offset sai) mà không bắt lỗi | Tra thêm `findByEmailIgnoreCase` (method mới trong `UserRepository`) + guard `passwordHash == null \|\| isBlank()` ⇒ trả `null` (HTTP 400) + bọc `BCrypt.checkpw` trong `try/catch (IllegalArgumentException)` |
| 9.26 | Bắn **3 request đăng nhập song song** thì 1 request trả **500 `error 112 (WriteConflict)`** tại `MongoSequenceService.nextId()` khi cấp `_id` cho `refresh_tokens` | Nhiều login cùng lúc `findOneAndUpdate` trên **cùng** document `counters` (trong transaction) ⇒ write conflict, driver retry nhưng request vẫn fail | **Chưa xử lý** (ngoài phạm vi lỗi đăng nhập): gửi lại request là 200; hướng khắc phục là `@Retryable`/retry vòng lặp cho `nextId()` |
